...
def test_get_torrent(response_mock, datafix_read, datafix_dir):
    ...
    with response_mock([
        ...
        torr = tracker.get_torrent('https://eniatv.com/viewtopic.php?t=1558', last_updated=None)
        ...
        assert torr.page.title == (
            'Пространство / Экспансия / The Expanse / Сезон: 5 / Серии: 1-9 из 10 (Брек Эйснер) '
            '[2020, Канада, США, фантастика, триллер, драма, детектив, WEB-DL 720p] MVO (LostFilm) + '
            'Original + Sub (Rus, Eng) :: eniahd.com'
        )
