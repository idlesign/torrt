...
1. Register your bot with BotFather as described at <https://core.telegram.org/bots#6-botfather>
  ...
  !!! note
      If you have already configured notifications with Telegram you don't need to create a new bot. Use an existing one.
...
3. Configure **torrt** to use the bot:
    ...
    Restrict users (comma-separated) talking to the bot with option `allowed_users`:
...
2. Create service definition in your home directory:
    ...
    Create `~/.config/systemd/user/torrt.service` (for example with a text editor) with:
